...
###############################################################################
#  • Base: Debian 12 (slim) + system Chromium (APT) → stable driver pairing
...
# -----------------------------------------------------------------------------
    ...
    # `set -eux` gives verbose, fail‑fast logs at build time.
    # -----------------------------------------------------------------------------
    RUN set -eux; \
    ...
    # • GPT_REVIEW_PROFILE stores persistent cookies (mount a host dir to persist)
    # -----------------------------------------------------------------------------
